...
4. 为具体实现事项派发功能线程
5. 一次验收一个阶段
6. 当现实变化时纠偏 Working Goal
7. 只在真实风险或边界变化时打断用户

功能线程负责一个明确交付单元，默认优先选择性价比高、速度快、足以完成任务的模型配置。

子代理服务于功能线程的降噪与并行，优先承担探索、日志归因、测试缺口扫描、风险扫描和总结。
...
- 每个阶段优先保持一个主要写入路径。
- 主线程不要直接吞具体实现、长日志和多轮修复噪音。
- 功能线程不追求默认使用最强模型，强模型优先保留给主编排、复杂判断和高风险验收。
