...
按当前阶段读取：[软件工程流程](references/software-engineering-workflow.md) 用于架构/契约、测试与目录；[并行执行与交接](references/autonomous-execution-workflow.md) 用于依赖、共享资源或接管；[角色矩阵](references/agent-role-matrix.md) 用于选角色。不要默认加载全部参考。
