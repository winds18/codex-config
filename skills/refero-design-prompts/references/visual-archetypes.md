...
用于把高完成度模板的视觉能力抽象为可复用方向。将原型作为选择提示；复用代码、素材或模板按实际许可证和用户授权处理。
...
- 方向明确时选择一个主原型；只有用户要求多方向探索时才展开多个关键视图。
...
- 视觉重构先检查渲染；发生差异时查看相关元素的 computed style，如颜色、字体、position、z-index、overflow、transform。
- 媒体引用应可解析且来源获准；图片提供适当 alt，媒体尺寸与布局稳定，按需设置 object-fit 和加载失败替代。
- 动效必须支持 `prefers-reduced-motion`，移动端降低 3D、粒子、parallax 或视频负载。
- 统一动效 token 和节奏，避免多个库或持续效果争抢注意力；允许目的清楚且相互协调的局部动效。
- 首屏应清楚表达真实身份、产品或作品；居中布局、光效或卡片是否合适以内容层级和用户目标判断，不作为绝对禁令。

## 来源边界

- 高层模式参考 [Personal Homepage Skill](https://github.com/shengjidaguai-china/personal-homepage-skill) 的公开仓库。
- 该项目快照采用非商业许可；借用具体模板、文本、素材或代码时遵守其限制，不能从本文件推断商业使用授权。
